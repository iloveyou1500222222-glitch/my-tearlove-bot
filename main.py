...
# 2. Member Track (ချိုချိုသာသာ/လွမ်းမောစရာ)
async def track_members(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if u.chat_member.new_chat_members:
        ...
        await chat.send_message(f"✨ မင်္ဂလာပါ {user.first_name} ရေ... ကျွန်မတို့ Group လေးကို နွေးထွေးစွာ ကြိုဆိုပါတယ်။ 🥰 ({time})")
        ...
        await chat.send_message(f"🥀 {user.first_name} ရေ... ထွက်သွားတော့မလား... မင်းမရှိတဲ့ ဒီနေရာလေးက အထီးကျန်နေခဲ့ပြီနော်။ ပြန်လာခဲ့ပါနော်။ 🕯️ ({time})")

# 3. Msg Handler (စာသင်တာ/ကြေညာတာ မဟုတ်ရင် Forward)
async def msg_handler(u: Update, c: ContextTypes.DEFAULT_TYPE):
    if not u.message: return
    # စာသင်ထားတာဆိုရင်
    if u.effective_chat.type == 'private' and u.message.text in teach_data:
        await c.bot.send_chat_action(chat_id=u.effective_chat.id, action="typing")
        await asyncio.sleep(5)
        await u.message.reply_text(f"💬 မေးခွန်း: {u.message.text}\n✅ အဖြေ: {teach_data[u.message.text]}\n⏱️ {get_time()}")
        return
    # ကျန်တဲ့စာ/ပုံ တွေ Forward ပို့
    if u.effective_chat.type == 'private':
        user = u.effective_user
        info = f"📩 **New Message**\n👤 {user.first_name}\n🆔 `{user.id}`\n🔗 @{user.username or 'No'}\n⏱️ {get_time()}"
...
async def bcast(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if not msg: await u.message.reply_text("⚠️ ကျေးဇူးပြု၍ ကြေညာမည့်စာကို ရိုက်ပေးပါရှင်။"); return
